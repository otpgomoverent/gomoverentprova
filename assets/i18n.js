...
(function(){
  ...
  function replaceTextNodesExact(root, map){
    ...
    while((n=w.nextNode())){
      ...
      if(map[t]!=null){ ops.push([n, raw.replace(t, map[t])]); }
    }
    ops.forEach(([node,val])=> node.nodeValue=val);
  }

  function replaceTextNodesPartial(root, map){
    const w=document.createTreeWalker(root, NodeFilter.SHOW_TEXT, null); let n;
    const ops=[];
    while((n=w.nextNode())){
      let raw=n.nodeValue; let changed=false;
      for(const [needle, repl] of Object.entries(map)){
        if(needle && raw.includes(needle)){ raw = raw.split(needle).join(repl); changed=true; }
      }
      if(changed) ops.push([n, raw]);
    }
    ops.forEach(([node,val])=> node.nodeValue=val);
  }
  ...
  function apply(dict){
    ...
    if(dict["__attrsReplace__"]){
      for(const [sel, attrs] of Object.entries(dict["__attrsReplace__"])){
        document.querySelectorAll(sel).forEach(el=>{
          for(const [a,map] of Object.entries(attrs)){
            const cur=el.getAttribute(a); if(cur && map[cur]!=null) el.setAttribute(a, map[cur]);
          }
        });
      }
    }

    if(dict["__replace__"]) replaceTextNodesExact(document.body, dict["__replace__"]);
    if(dict["__replacePartial__"]) replaceTextNodesPartial(document.body, dict["__replacePartial__"]);
  ...
  function inject(){
    const wrap=document.createElement("div"); Object.assign(wrap.style,{position:"fixed",top:"12px",right:"12px",zIndex:"9999",background:"rgba(255,255,255,.9)",border:"1px solid #ddd",borderRadius:"10px",padding:"6px 8px",boxShadow:"0 4px 16px rgba(0,0,0,.08)",fontFamily:"Inter,system-ui,-apple-system,Segoe UI,Roboto,Arial,sans-serif",fontSize:"14px"});
